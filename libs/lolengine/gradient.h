...
//source file: https://github.com/lolengine/lol/blob/2c61a5e80521c91e52c2c1ad8a1e621e43ae1a1a/include/lol/private/math/gradient.h
...
protected:
    ...
    //N hardcoded to 1 in this case
    static int constexpr gradient_count = 1 << std::min(std::max(1 + 2, 5), 20);

    float get_gradient(int origin) const
    {
        /* Quick shuffle table:
        ...
        {
            std::array<float, gradient_count> ret;
            for (int k = 0; k < gradient_count; ++k)
            {
                //float v = rand(-1.f, 1.f);
